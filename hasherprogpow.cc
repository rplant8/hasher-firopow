...
NAN_METHOD(hash_one) {
        ...
        if (info.Length() < 3)
            return THROW_ERROR_EXCEPTION("hasher-progpow-firo.hash_one - 3 arguments expected.");
...
NAN_METHOD(verify) {
        ...
        if (info.Length() < 5)
            return THROW_ERROR_EXCEPTION("hasher-progpow-firo.verify - 5 arguments expected.");
...
NODE_MODULE(hasherprogpow, init)